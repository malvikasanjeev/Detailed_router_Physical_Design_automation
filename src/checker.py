...
class Net:
  def __init__(self, net, insts, pins, idx):
    ...
    self._netsol = [] #list store the track for now
          ...
          if net._id != nbr.object:
            if overlaps(rb, nbr.bbox):
              ...
              if nbr.object < len(nets):
                ...
                if(net._name,nets[nbr.object]._name) not in spacingnets and (nets[nbr.object]._name,net._name) not in spacingnets:
                  spacingnets.append((net._name,nets[nbr.object]._name))
                #spacingViolNets.append((net._name, Rect(rb.ll.x, rb.ll.y, rb.ur.x, rb.ur.y)))
                #spacingViolNets.append((nets[nbr.object]._name,[round(i) for i in nbr.bbox]))
                ...
                spacingViol.append(Rect(rb.ll.x, rb.ll.y, rb.ur.x, rb.ur.y))
                #spacingViolNets.append((net._name, Rect(rb.ll.x, rb.ll.y, rb.ur.x, rb.ur.y)))
                spacingnets.append((net._name,'obst'))
              else:
                assert(0)

  numViolations //= 2
  #print(f"spacingviolnets:{spacingViolNets}")
  
  #print(f"spacingnets:{spacingnets}")
  return numViolations, (widthViol, spacingViol), spacingnets
  ...
  for net in nets:
    ...
    if len(G.nodes()):
      if not nx.is_connected(G):
        numOpens += 1
        openset.add(net._name)

  return numOpens,openset

# check for spacing and connectivity violations
def check(nets, insts, obsts):
  layerTrees = buildTree(nets, insts, obsts)
  nViols, violations,spacingnets= checkSpacing(layerTrees, nets, insts)
  print(f"Total number of spacing violations : {nViols}")
  #print(f"spacing violations: {violations[1]}")

  nOpens,openset = checkConnectivity(layerTrees, nets, insts)
  print(f"Total number of nets : {len(nets)}")
  print(f"Total number of open nets : {nOpens} {openset}")

  return violations,spacingnets,openset
...
def loadAndCheck(odef, idef, lef, plot):
  ...
  #print(f"layerwidth:{layerWidth}, layerspacing:{layerSpacing}")
  ...
  violations,spacingnets,openset = check(nets, insts, obsts)
  if (plot):
    track = dict()
  ...
  return netDict,insts,pins,obsts,nets,spacingnets,openset
